  ...
  try {
    ...
    async function getChampionName(name) {
      ...
      try {
        ...
        if (championId === undefined) throw new Error('Campeão não encontrada, tente escrever de maneira não burra.')
      } catch (error) {
        return
      }
    ...
    if (championQuery === undefined) {
      message.channel.send('Campeão não encontrada, tente escrever de maneira não burra.')
      throw new Error('Campeão não encontrada, tente escrever de maneira não burra.')
    }

    if (laneQuery === "Lane não encontrada.") {
      message.channel.send('Lane não encontrada, tente escrever de maneira não burra.')
      throw new Error('Lane não encontrada, tente escrever de maneira não burra.')
    }

    if (tier === "Elo não encontrado.") {
      message.channel.send('Elo não encontrado, tente escrever de maneira não burra.')
      throw new Error('Elo não encontrado, tente escrever de maneira não burra.')
    }
    ...
    try {
      ...
      throw new Error('Algo deu com a verificação da build do campeão, tente novamente.')
    }
